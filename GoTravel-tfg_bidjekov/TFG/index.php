...
// Procesar la solicitud del formulario de opinión
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    // Verificar si los campos del formulario de opinión están definidos
    if (isset($_POST['nombre'], $_POST['apellidos'], $_POST['correo'], $_POST['mensaje'], $_POST['rating'])) {
        $nombre = $_POST['nombre'];
        ...
        $sql = "INSERT INTO usuario (Nombre, Apellido, Email, Opinion) VALUES (?, ?, ?, ?)";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("ssss", $nombre, $apellidos, $correo, $mensaje);

        if ($stmt->execute()) {
            $successMessage = "Gracias por tu opinión. ¡La hemos recibido!";
        } else {
            $errorMessage = "Error al guardar la opinión. Por favor, intenta nuevamente.";
        }

        $stmt->close();
    }

    // Aquí sigue el bloque de procesamiento de la búsqueda de hoteles y vuelos (lo que ya tenías)
    if (isset($_POST['destino']) && isset($_POST['tipo_estancia'])) {
        ...
        // Consulta según el tipo de estancia (Hotel o Vuelo)
...
<div class="text-center mt-3 pt-3" id="opina">
    ...
    <h3 class="text-dark">Déjanos tu opinión</h3>
</div>
